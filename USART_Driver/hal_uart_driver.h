...
/* Macros to enable the clocks for various UART - Page 183*/
...
typedef struct
{
	uint32_t BaudRate;      /* UART communication baudrate */															 
	uint32_t WordLength;    /* Number of data bits transmitted or received in a frame */													 
	uint32_t StopBits;      /* Number of stop bits transmitted */														
	uint32_t Parity;        /* Parity mode. */
	uint32_t Mode;          /* Whether the Receive or Transmit mode is enabled or disabled */
	uint32_t OverSampling;  /* Over sampling 8 is enabled or disabled */
}uart_init_t;
