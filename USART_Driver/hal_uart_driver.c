...
static void hal_uart_clear_error_flag(uart_handle_t *huart)
{
	...
	tmpreg = huart->Instance->SR; // Reading operation to SR -> clear it to 0x00
	tmpreg = huart->Instance->DR; // Reading operation to DR -> clear it to 0x00       
}
